...
function scoreLanding(_sections: ArtifactSection[], fullText: string): ArtifactQualityScore {
  ...
  dimensions.push({ label: "Clarity", score: headline ? 82 : 50, reason: headline ? "Headline present" : "Headline unclear" });
  dimensions.push({ label: "Conversion strength", score: cta ? 86 : 48, reason: cta ? "CTA visible" : "CTA weak or missing" });
  dimensions.push({ label: "Trust strength", score: proof ? 80 : 55, reason: proof ? "Proof elements present" : "Trust/proof thin" });
...
function scoreFinancialTable(sections: ArtifactSection[]): ArtifactQualityScore {
  ...
  const t = table.content as import("./artifactTypes.js").ArtifactTable;
  ...
  const colOk = t.columns.length >= 2;

  dimensions.push({
    label: "Data completeness",
    score: colOk && !emptyRows ? 88 : 45,
    reason: colOk && !emptyRows ? "Table has structure" : "Table structure weak",
  });
  dimensions.push({
    label: "Format fit",
...
function scoreGeneric(sections: ArtifactSection[], fullText: string): ArtifactQualityScore {
  ...
  return {
    ...
    suggestedFixes:
      wc < 80
        ? [{ id: "expand", label: "Expand key sections", severity: "minor" }]
        : [],
  };
}
