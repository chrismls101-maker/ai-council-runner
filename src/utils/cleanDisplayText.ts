...
export function promptRequestsMarkdown(prompt: string | null | undefined): boolean {
  if (!prompt) return false;
  return /\b(in markdown|as markdown|markdown format|\.md\b|code block)\b/i.test(prompt);
}

/** Strip decorative markdown noise for plain-answer display. */
export function cleanDisplayText(text: string, options?: { preserveCodeBlocks?: boolean }): string {
  if (!text?.trim()) return "";
