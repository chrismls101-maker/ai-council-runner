...
When the user toggles QA Mode on, a **sticky dismissible notification card** appears on first activation per app session. This is not a toast. It is a deliberate, premium Glass-styled moment that tells the user exactly what mode they have entered.

### Notification design (v1.8 — current)

The card appears centered in the Glass IDE stream pane. It does **not** auto-dismiss. The user explicitly dismisses via:

- **Turn on for this session** — dismiss and keep QA Mode enabled
- **Not now** — dismiss

Copy positions QA as a **local shipping gate** for JavaScript/TypeScript projects. Skipped checks are always disclosed in the VERIFY phase.

```
┌─────────────────────────────────────────────────┐
│   ◈  QA Mode: local shipping gate              │
│                                                 │
│   After Glass edits code, QA Mode runs local    │
│   checks, runtime smoke, and two AI reviews     │
...
│   ◎  Two-pass AI review                         │
│                                                 │
│   Skipped checks are always disclosed.          │
│                                                 │
│   [ Turn on for this session ]  [ Not now ]    │
└─────────────────────────────────────────────────┘
```
...
- **No countdown bar** — manual dismiss only (v1.8)
