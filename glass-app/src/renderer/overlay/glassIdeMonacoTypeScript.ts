...
import { registerGlassIdeProjectLibs, resetGlassIdeProjectLibs } from "./glassIdeMonacoProjectLibs.ts";
...
export async function configureGlassIdeTypeScript(
  ...
  resolvedProjectRoot = res.projectRoot ?? null;
  if (key) {
    void registerGlassIdeProjectLibs(key);
  } else {
    resetGlassIdeProjectLibs();
  }
  return res;
