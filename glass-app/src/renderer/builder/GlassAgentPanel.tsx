...
function AgentCard({
  ...
  const opensDedicatedWorkspace = agentOpensDedicatedWorkspace(def.id);
  ...
  const handleCardClick = useCallback((): void => {
    ...
    if (def.id === "research") {
      window.glass.openResearchExplorer("");
      onDismissPanel?.();
      return;
    }
    if (def.id === "code") {
      window.glass.openCodeAnalystExplorer("");
      onDismissPanel?.();
      return;
    }
    if (def.id === "writing") {
      window.glass.openWritingStudio("");
      onDismissPanel?.();
      return;
    }
    setExpanded((prev) => {
  ...
  }, [isRunning, def.id, onOpenIde, onDismissPanel]);
  ...
  const needsWorkspace = agentRequiresCodeWorkspace(def.id);
    ...
    >
      <div className="gap-card__header">
        ...
        >
          ...
          {!isRunning && !opensDedicatedWorkspace ? (
      ...
      {expanded && !isRunning && !opensDedicatedWorkspace && (
      ...
      <div className="gap-body">
        ...
        {GLASS_AGENT_CATALOG.map((def) => (
          <AgentCard
            ...
            onDismissPanel={
              def.id === "research" || def.id === "code" || def.id === "writing" ? onClose : undefined
            }
          />
        ))}
      </div>
