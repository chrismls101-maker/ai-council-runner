...
export function SystemAudioLiveMeter({
  ...
  return (
    <div className="audio-live-meter" data-testid="glass-system-audio-live-meter">
      <div className="audio-live-meter__bars" aria-hidden="true">
        {levels.map((level, index) => (
          <span
            key={index}
            className="audio-live-meter__bar"
            style={{ height: `${Math.max(8, Math.round(level * 100))}%` }}
          />
        ))}
      </div>
      <p
        className={`audio-live-meter__status audio-live-meter__status--${phase}`}
