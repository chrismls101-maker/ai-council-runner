...
import { getOverlayVoiceController } from "./overlayVoiceController.ts";
 ...
 * Routes through VoiceController's chorus + reverb chain (same character as Sorting Hat).
 */
export function useIivoTtsRequest(): {
  ...
  useEffect(() => {
    ...
    const runHtmlFallback = (): void => {
      const voice = getOverlayVoiceController();
      void voice.resumeContext().then(() => {
        ...
        voice.connectAudioElement(audio, { processed: true });
