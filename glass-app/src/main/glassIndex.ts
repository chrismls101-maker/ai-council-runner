...
function openDb(projectRoot: string): Database.Database {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS symbol_index_name ON symbol_index(name);
    CREATE INDEX IF NOT EXISTS symbol_index_path ON symbol_index(path);
  `);
...
async function indexFileUnlocked(projectRoot: string, filePath: string): Promise<boolean> {
  ...
  const wrote = writeIndexEntry(db, resolved, relPath, hash, embedding);
  if (wrote && /\.tsx?$/i.test(resolved)) {
    try {
      writeSymbolIndex(db, resolved, relPath, content);
    } catch {
      /* best-effort */
    }
  }
...
export function removeFromIndex(projectRoot: string, filePath: string): void {
  try {
    ...
    db.prepare("DELETE FROM symbol_index WHERE path = ?").run(resolved);
  } catch {
    /* best-effort */
  }
}
...
export interface GlassSymbolHit {
  file: string;
  relPath: string;
  name: string;
  kind: string;
...
function scanSymbolsFromContent(content: string): Array<{ name: string; kind: string; line: number }> {
...
function writeSymbolIndex(
  ...
  db.prepare("DELETE FROM symbol_index WHERE path = ?").run(resolved);
  const symbols = scanSymbolsFromContent(content);
  if (!symbols.length) return;
  const insert = db.prepare(
    "INSERT INTO symbol_index (path, rel_path, name, kind, line) VALUES (?, ?, ?, ?, ?)",
  );
  const write = db.transaction((
    absPath: string,
    rel: string,
    rows: Array<{ name: string; kind: string; line: number }>,
  ) => {
    for (const row of rows) {
      insert.run(absPath, rel, row.name, row.kind, row.line);
    }
  });
  write(resolved, relPath, symbols);
}
...
export function searchSymbols(
  ...
  const tokens = needle.split(/[^a-z0-9_$]+/i).filter((t) => t.length >= 2);
  if (!tokens.length) return [];

  try {
    const db = openDb(projectRoot);
    const rows = db.prepare(
      "SELECT path, rel_path, name, kind, line FROM symbol_index",
    ).all() as Array<{
      path: string;
      rel_path: string;
      name: string;
    ...
    const scored = rows
      .map((row) => {
        ...
        for (const token of tokens) {
          ...
          else if (nameLower.startsWith(token)) score += 70;
      ...
      .filter((r): r is GlassSymbolHit & { score: number } => r !== null)
      .sort((a, b) => b.score - a.score || a.name.localeCompare(b.name))
      .slice(0, topN);

    return scored.map(({ file, relPath, name, kind, line }) => ({
      file, relPath, name, kind, line,
    }));
  } catch (err) {
    console.warn("[glassIndex] searchSymbols failed:", err);
    return [];
  }
}
