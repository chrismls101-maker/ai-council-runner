...
export function formatMentionedFiles(relPaths: string[]): string {
  if (relPaths.length === 0) return "";
  return [
    "User @-mentioned files (read these first):",
    ...relPaths.map((p) => `- ${p}`),
  ].join("\n");
}

export function formatSymbolHits(
  hits: Array<{ relPath: string; name: string; kind: string; line: number }>,
): string {
  if (hits.length === 0) return "";
  return [
    "Relevant symbols from project index:",
    ...hits.map((h) => `- ${h.kind} ${h.name} (${h.relPath}:${h.line})`),
  ].join("\n");
}
...
export async function buildCoderBootstrapContext(opts: {
  ...
  symbolHits?: Array<{ relPath: string; name: string; kind: string; line: number }>;
  ...
  if (glassContext) {
    parts.push(
      "[GLASS_CONTEXT.md — Project memory file in this repo (user-generated). Not part of the Glass app.]",
      glassContext,
      "---",
    );
  }
  ...
  if (opts.gitContext?.trim()) {
    parts.push("--- Git context (read-only) ---");
    parts.push(opts.gitContext.trim());
  }

  if (opts.mentionedFiles && opts.mentionedFiles.length > 0) {
    parts.push(formatMentionedFiles(opts.mentionedFiles));
    const mentionSnippets = await Promise.all(
      opts.mentionedFiles.slice(0, 6).map(async (rel) => {
        try {
          const abs = path.join(path.resolve(expandAgentPath(projectRoot)), rel);
          const raw = await fsp.readFile(abs, "utf-8");
          const content = raw.length > 2_500
            ? `${raw.slice(0, 2_500)}\n… [truncated]`
            : raw;
          return `--- @${rel} ---\n${content}`;
        } catch {
    ...
    for (const snippet of mentionSnippets) {
      if (snippet) parts.push(snippet);
    }
  }

  if (opts.symbolHits && opts.symbolHits.length > 0) {
    parts.push(formatSymbolHits(opts.symbolHits));
  }
