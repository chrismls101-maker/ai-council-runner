...
/** Terminal beside the left rail opens near the work-area top (dock stays put). */
export const GLASS_TERMINAL_RAIL_TOP_INSET_PX = 24;

/** Terminal beside the left rail opens to the right of the dock, near the work-area top. */
export function terminalWindowBoundsBesideDock(
  dockBounds: Rectangle,
  panelWidth = GLASS_TERMINAL_DEFAULT_WIDTH,
  panelHeight = GLASS_TERMINAL_DEFAULT_HEIGHT,
  workArea?: Rectangle,
  ...
  const width = panelWidth + pad * 2;
  const height = panelHeight + pad * 2;
  let x = dockBounds.x + dockBounds.width + gap;
  let y = workArea ? workArea.y + GLASS_TERMINAL_RAIL_TOP_INSET_PX : dockBounds.y;

  if (workArea) {
    const edge = GLASS_TERMINAL_RAIL_TOP_INSET_PX;
    const maxX = workArea.x + workArea.width - width - edge;
    x = Math.max(workArea.x + edge, Math.min(x, maxX));
    const maxY = workArea.y + workArea.height - height - edge;
    y = Math.max(workArea.y + edge, Math.min(y, maxY));
  }
