...
test("default settings use cmd-shift-space and primary display", () => {
  ...
  assert.equal(DEFAULT_GLASS_USER_SETTINGS.dockPlacement, "left-rail");
...
test("parseDockPlacement accepts top or left-rail", () => {
  assert.equal(parseDockPlacement("top"), "top");
  assert.equal(parseDockPlacement("left-rail"), "left-rail");
  assert.equal(parseDockPlacement("right-rail"), "left-rail");
  assert.equal(parseDockPlacement(undefined), "left-rail");
});
