...
import {
  ...
  MACOS_DOCK_HIDDEN_CLEARANCE_PX,
...
test("overlay sits near screen bottom when macOS dock is autohidden (flush workArea)", () => {
  const overlay = overlayLayoutFromDisplay(macBookBuiltIn);
  assert.equal(overlay.height, macBookBuiltIn.workArea.height - MACOS_DOCK_HIDDEN_CLEARANCE_PX);
});

test("macOS visible frame work area extends overlay when dock autohides", () => {
  const dockHidden: DisplayLayoutContext = {
    ...macBookBuiltIn,
    workArea: { x: 0, y: 30, width: 1440, height: 870 },
    workAreaSource: "macos-visible-frame",
  };
  const overlay = overlayLayoutFromDisplay(dockHidden);
  assert.equal(overlay.height, 870);
});

test("overlay respects macOS dock strip when workArea shrinks for visible dock", () => {
  const dockReserved: DisplayLayoutContext = {
    ...macBookBuiltIn,
    workArea: { x: 0, y: 30, width: 1440, height: 798 },
  };
  const overlay = overlayLayoutFromDisplay(dockReserved);
  assert.equal(overlay.height, dockReserved.workArea.height);
});
