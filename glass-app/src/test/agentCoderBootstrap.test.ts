...
test("buildCoderBootstrapContext prepends GLASS_CONTEXT before other sections", async () => {
  ...
  try {
    ...
    assert.ok(ctx);
    assert.match(ctx, /^\[GLASS_CONTEXT\.md — Project memory file in this repo/);
...
test("formatSemanticPreSeedFiles lists ranked paths for index hits", () => {
  const text = formatSemanticPreSeedFiles(["src/auth.ts", "src/api.ts"]);
  assert.match(text, /Relevant files identified by semantic search/);
  assert.match(text, /src\/auth\.ts/);
  assert.match(text, /src\/api\.ts/);
});

test("buildCoderBootstrapContext uses semantic pre-seed instead of file walk", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "glass-preseed-"));
  try {
    await mkdir(path.join(root, "src"), { recursive: true });
    await writeFile(path.join(root, "src", "main.ts"), "export {};\n", "utf-8");
    const ctx = await buildCoderBootstrapContext({
      projectRoot: root,
      preSeedFiles: ["src/main.ts"],
      includeFileWalk: false,
    });
    assert.ok(ctx);
    assert.match(ctx, /Relevant files identified by semantic search/);
    assert.doesNotMatch(ctx, /Project file index/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
