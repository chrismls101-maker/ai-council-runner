...
test("API key values stay in main process and out of GlassState", () => {
  ...
  const panel = readFileSync(join(ROOT, "renderer", "builder", "ApiKeyManagerPanel.tsx"), "utf8");

  assert.match(store, /safeStorage\.encryptString/);
  assert.match(store, /safeStorage\.decryptString/);
  assert.match(store, /normalizeApiKeyMeta/);
